...
export default function DocumentDetailsPage() {
  ...
  const actionButtonClass =
    'inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900';

  const viewButton = (
    <button type="button" onClick={handleView} className={actionButtonClass}>
  ...
  const downloadButton = (
    <button type="button" onClick={handleDownload} className={actionButtonClass}>
  ...
  return (
    ...
    >
      ...
      {previewOpen ? (
        ...
        >
          <div
            className="flex h-[92vh] w-full max-w-7xl flex-col overflow-hidden rounded-3xl border border-white/70 bg-white shadow-[0_30px_80px_rgba(15,23,42,0.35)]"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-4 border-b border-slate-200/80 px-5 py-4 sm:px-6">
              <div className="min-w-0">
                <p className="text-[11px] font-medium uppercase tracking-[0.22em] text-slate-500">Preview</p>
                <h3 className="mt-1 truncate text-lg font-semibold text-slate-900">{previewFilename || document.originalFilename}</h3>
                <p className="mt-1 truncate text-sm text-slate-500">{previewMimeType || document.mimeType}</p>
              </div>
              <div className="flex flex-shrink-0 items-center gap-2">
                <button
                  type="button"
                  onClick={handleDownload}
                  className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:border-slate-300 hover:bg-slate-100 hover:text-slate-900"
                >
                  <Download className="h-4 w-4" />
                  Download File
                </button>
                <button
                  type="button"
                  onClick={closePreview}
                  className="inline-flex rounded-xl border border-slate-200 bg-white p-2.5 text-slate-500 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900"
            ...
            <div className="flex-1 bg-slate-100 p-3 sm:p-6">
              <div className="flex h-full items-center justify-center overflow-hidden rounded-2xl border border-slate-200 bg-white">
                {previewLoading ? (
                  ...
                  <img
                    src={previewUrl}
                    alt={previewFilename || document.originalFilename}
                    className="h-full w-full object-contain"
                  />
                ) : previewUrl ? (
                  <iframe
                    src={previewUrl}
                    title={previewFilename || document.originalFilename}
